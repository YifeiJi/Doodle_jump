cc.Class({
  ...
  start () {
    // window.player_type = 'winter' // 已经在start.js中指定
  ...
  RandomproduceBlock: function (input) {
    ...
    while (this.maxBlockHeight < this.line + 3 * this.maxY) {
      ...
      if (Math.random() > 0.6) randy = y + Math.random() * this.interval_b + 40; else { randy = y + Math.random() * this.interval_b + 40}
      ...
      if ((Math.random() > 0.9) && (this.maxBlockHeight > 10 * this.maxY)) {
        ...
        if ((player_type === 'default') || (player_type === 'jungle')) {
          ...
          if (Math.random() > 0.8) { newmonster.getComponent('Monster').type = 'move'}
        ...
        if ((player_type === 'winter') || (player_type === 'underwater')) {
          ...
          if (Math.random() > 0.8) { newmonster.getComponent('WinterMonster').type = 'move'}
...
*/
  update: function (dt) {
    ...
    // this.line=this.line-this.speed*dt+this.speed_block*dt;
    if (this.player.getComponent('Player').alive === true) { this.scoreDisplay.string = 'Score: ' + parseInt((this.line + this.maxY) / 10)}
  }
