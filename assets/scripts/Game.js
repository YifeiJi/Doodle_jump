cc.Class({
  ...
  resume: function (e) {
    ...
    if (this.player.getComponent('Player').onhat === true) { this.player.getComponent('Player').hatsound = cc.audioEngine.play(this.player.getComponent('Player').hat_audio, true, 0.5) }
    if (this.player.getComponent('Player').onrocket === true) { this.player.getComponent('Player').rocketsound = cc.audioEngine.play(this.player.getComponent('Player').rocket_audio, true, 0.5) }
  ...
  pause: function (e) {
    ...
    if (this.player.getComponent('Player').onrocket === true) { cc.audioEngine.stop(this.player.getComponent('Player').rocketsound) }
    if (this.player.getComponent('Player').onhat === true) { cc.audioEngine.stop(this.player.getComponent('Player').hatsound) }
    var q = this.pausebutton.node.children[0].children[0]
  ...
  set_level: function () {
    ...
    if (window.level === undefined) { window.level = 'medium' }
  ...
  start () {
    ...
    // fitHeight=true;
    // bg.fitWidth=true;
    this.pausebutton.node.setPosition(this.maxX - 1.5 * this.pausebutton.node.width, -this.maxY + this.pausebutton.node.height)
    this.pausebutton.node.zIndex = 5

    // this.scoreDisplay.setPosition(-this.maxX +this.scoreDisplay.width, -this.maxY + this.scoreDisplay.height);
    // this.reviveDisplay.setPosition(-this.maxX +this.scoreDisplay.width, -this.maxY +20 +this.scoreDisplay.height+this.reviveDisplay.height);
    this.pausebutton.node.on(cc.Node.EventType.TOUCH_START, this.pause, this)
    ...
    if (window.reviveNumber === undefined) { window.reviveNumber = 0 }
    ...
    if (window.rocketNumber === undefined) { window.rocketNumber = 0 }

    if (window.hatNumber === undefined) { window.hatNumber = 0 }
    ...
    if (window.sensibility === undefined) { window.sensibility = 'medium' }
    // console.log('sen')
    // console.log(window.sensibility)
    if (window.sensibility === 'low') { newplayer.getComponent('Player').set_low_sensibility() } else if (window.sensibility === 'medium') { newplayer.getComponent('Player').set_medium_sensibility() } else if (window.sensibility === 'high') { newplayer.getComponent('Player').set_high_sensibility() }
  ...
  RandomproduceBlock: function (input) {
    ...
    while (this.maxBlockHeight < this.line + 4 * this.maxY) {
      ...
      if (Math.random() > this.param_basic) {
        ...
        else { newBlock = cc.instantiate(this.basicblock) }
        ...
        else { newBlock = cc.instantiate(this.leftrightblock) }
      ...
      if ((Math.random() > this.param_monster) && (this.maxBlockHeight > 10 * this.maxY)) {
        ...
        if (randy >= this.last_monster + this.monster_interval) {
          if ((player_type === 'winter') || (player_type === 'underwater')) { var newmonster = cc.instantiate(this.wintermonsterfab) } else { var newmonster = cc.instantiate(this.monsterfab) }

          this.node.addChild(newmonster, 1)
          if ((player_type === 'default') || (player_type === 'jungle')) {
            newmonster.getComponent('Monster').game = this
            if (Math.random() > this.param_monster_move) { newmonster.getComponent('Monster').type = 'move' }
          ...
          if ((player_type === 'winter') || (player_type === 'underwater')) {
            ...
            if (Math.random() > this.param_monster_move) { newmonster.getComponent('WinterMonster').type = 'move' }
      ...
      if ((Math.random() > this.param_loch) && (this.maxBlockHeight > 15 * this.maxY)) {
        ...
        newloch.setPosition(randx - newloch.width * 0.5, randy - this.line - this.maxY, 0)
...
*/
  update: function (dt) {
    if (this.player.getComponent('Player').fall === false) {
      ...
      if (this.player.y > 0) { this.speed = -((this.player.y + this.maxY) / (this.maxY * 1.5)) * this.player.getComponent('Player').speedy }
    ...
    if (this.speed_block > 0) { this.speed_block = this.speed_block - this.acc_block * dt }
    if (this.speed_block < 0) { this.speed_block = 0 }
    // this.line=this.line-this.speed*dt+this.speed_block*dt;
    if (this.player.getComponent('Player').alive === true) {
      if ((this.line + this.maxY) / 10 > this.score) { this.score = parseInt(this.line + this.maxY) / 10 }
